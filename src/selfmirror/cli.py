...
from selfmirror.llm.service import _background_admission_bypass
from selfmirror.published_time import format_published_time
...
from selfmirror.soul.preference_analyzer import DEFAULT_PREFERENCE_EVENT_CHUNK_SIZE
...
def _bootstrap_container_runtime() -> None:
    ...
    from selfmirror.docker_runtime import bootstrap_runtime_environment
...
def _content_author_row(content: Any) -> tuple[str, str]:
    ...
    * **Label** — mirrors ``formatRecommendationAuthorLine`` in
      ...
      their old label.
    """
    from selfmirror.saved_sync.identity import canonical_source_platform
...
def _initialize_logging(log_level_override: str | None = None) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.logging_setup import configure_logging
...
def _build_registry() -> Any:
    ...
    from selfmirror.config import load_config
    from selfmirror.llm import build_llm_registry
...
def _build_browser() -> Any:
    ...
    from selfmirror.bilibili.browser import BilibiliBrowser
    from selfmirror.config import load_config
...
def _build_bilibili_client() -> Any:
    ...
    from selfmirror.bilibili.api import BilibiliAPIClient
    from selfmirror.bilibili.auth import resolve_runtime_cookie
    from selfmirror.config import load_config
...
def _build_soul_engine() -> Any:
    ...
    from selfmirror.config import load_config
    from selfmirror.llm.service import module_overrides_from_config
    from selfmirror.soul.engine import SoulEngine
...
def _build_recommendation_engine() -> Any:
    ...
    from selfmirror.config import load_config
    from selfmirror.llm.service import LLMService, module_overrides_from_config
    from selfmirror.recommendation.engine import (
...
def _build_dialogue(soul_engine: Any) -> Any:
    ...
    from selfmirror.soul.dialogue import DialogueLearningMode, SocraticDialogue
...
def _run_api_server(*, host: str = "127.0.0.1", port: int = 8420) -> None:
    ...
    from selfmirror.api.app import create_app
    ...
    from selfmirror.runtime.api_server import (
...
def _build_discovery_engine() -> Any:
    ...
    from selfmirror.discovery.engine import (
        ContentDiscoveryEngine,
        DiscoveryConcurrencyController,
    )
    from selfmirror.discovery.strategies.strategies import (
    ...
    from selfmirror.llm.concurrency import background_llm_concurrency
    from selfmirror.llm.service import LLMService, module_overrides_from_config
...
def _get_runtime_database() -> Any:
    ...
    config = load_config()
    database = Database(config.data_path / "selfmirror.db")
    database.initialize()
...
def _build_usage_recorder() -> Any:
    ...
    CLI-run LLM call was invisible in ``selfmirror cost``.
    """
    cached = _RUNTIME_COMPONENTS.get("usage_recorder")
    if cached is not None:
        return cached

    from selfmirror.llm.usage_recorder import UsageRecorder
...
def _runtime_database_path() -> Path:
    ...
    config = load_config()
    return config.data_path / "selfmirror.db"
...
def _maybe_create_runtime_database_backup() -> None:
    from selfmirror.storage.maintenance import maybe_create_scheduled_backup
...
def _create_reinit_backup() -> Path | None:
    ...
    try:
        ...
        from selfmirror.storage.maintenance import create_database_backup
        ...
        logging.getLogger("selfmirror.cli").warning("re-init backup failed", exc_info=True)
        return None


def _ensure_runtime_database_healthy() -> None:
    from selfmirror.storage.maintenance import check_database_integrity
    ...
    _print_status_panel(
        "error",
        "数据库损坏",
        "检测到本地数据库损坏，请先执行 `selfmirror db-repair` 再启动服务。",
    )
    if report.error:
...
def _run_db_repair() -> Any:
    from selfmirror.storage.maintenance import repair_database
...
def _history_item_to_event(item: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from selfmirror.sources.event_format import SOURCE_BILIBILI, build_event
...
def _x_tweet_to_event(tweet: dict[str, Any], *, event_type: str) -> dict[str, Any] | None:
    ...
    """
    from selfmirror.sources.event_format import SOURCE_TWITTER, build_event
...
def _save_embedding_config(
    *,
    ...
    """
    from selfmirror.config import load_config_with_diagnostics, save_config
...
def _interactive_embedding_setup(default_provider: str, *, auto_if_ready: bool = False) -> None:
    ...
    if auto_if_ready and _ollama_is_running() and _ollama_has_model("bge-m3"):
        ...
        console.print(
            ...
            "`selfmirror setup-embedding`。[/dim]"
        )
        return
    console.print(
    ...
    if choice in {"3", "follow"}:
        ...
        console.print(
            ...
            "`selfmirror setup-embedding` 单独配置。[/green]"
        )
        return
...
def _interactive_auth_setup(auth_manager: Any) -> Any:
    ...
    console.print(
        ...
        "     选这条会先退出 init；扩展同步完再跑 `selfmirror init` 即可。\n\n"
    ...
    if choice in {"1", "extension", "ext", ""}:
        console.print(
            ...
            "  1. 启动后端：[cyan]selfmirror start[/cyan]（或保持当前 docker compose up）\n"
            "  2. 装扩展：[cyan]https://github.com/whiteguo233/OpenBiliClaw/releases[/cyan]\n"
            "  3. 确认你已登录 B 站；扩展会几秒内同步 Cookie\n"
            "  4. 再跑 [cyan]selfmirror init[/cyan] 完成画像生成 + 首轮发现\n"
        )
...
def _prepare_init_runtime(*, require_bili_auth: bool = True) -> Any:
    ...
    if not _is_interactive_terminal():
        ...
        console.print("请先执行 `selfmirror auth login` 完成 B 站认证。")
        raise typer.Exit(code=1)
...
async def _run_init_discovery_backfill_async(
    ...
    *,
    ...
    from selfmirror.discovery.pool_snapshot import build_cold_start_pool_snapshot
    from selfmirror.runtime.refresh import InitialPoolUnavailableError
...
def _enqueue_xhs_bootstrap_task(
    *,
    ...
    from selfmirror.sources import source_bootstrap
...
def _collect_linuxdo_bootstrap_events(
    ...
    *,
    ...
    terminal_status = str(result.get("_selfmirror_terminal_status", "")).strip()
...
def _enqueue_zhihu_search_task(
    ...
    *,
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.zhihu_tasks import ZhihuTaskQueue
...
def _enqueue_zhihu_discovery_candidates(items: list[dict[str, Any]]) -> tuple[int, list[Any]]:
    ...
    from selfmirror.sources.zhihu_tasks import zhihu_discovery_items_to_contents
...
def _enqueue_reddit_discovery_candidates(
    ...
    *,
    ...
    from selfmirror.discovery.candidate_pool import discovered_content_to_candidate_write
    from selfmirror.sources.reddit_tasks import reddit_items_to_contents
...
def _collect_v2ex_bootstrap_events(
    ...
    *,
    ...
    if not events:
        return [], counts, "no_profile_signal_sources"
    raw_status = str(result.get("_selfmirror_terminal_status", "") or "").strip()
...
def _enqueue_reddit_discovery_task(
    ...
    *,
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.reddit_tasks import RedditTaskQueue
...
def _is_reddit_extension_backend(backend: str) -> bool:
    return str(backend or "").strip().lower() in {"extension", "selfmirror", "plugin"}
...
def _enqueue_dy_search_task(
    ...
    *,
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.dy_tasks import DyTaskQueue
...
def _drop_events_already_imported(
    ...
    if database is None or not events:
        return events, 0
    logger = logging.getLogger("selfmirror.cli")
...
def _favorites_to_history_rows(favorites: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    """
    from selfmirror.sources.event_format import SOURCE_BILIBILI
...
def _embedding_cache_runtime(config: Any) -> tuple[Any, set[str]]:
    ...
    """
    from selfmirror.llm.embedding import EmbeddingCache
...
def embedding_cache_stats() -> None:
    ...
    if stats["legacy_rows"]:
        _print_status_panel(
            ...
            "运行 `selfmirror embedding-cache-clean --apply` 可迁移并回收失效 namespace。",
        )
...
def embedding_cache_clean(
    ...
    if compact_report and not compact_report.get("ok"):
        _print_status_panel(
            ...
            f" {cache_path}（缓存可重建）后再运行 `selfmirror embedding-cache-clean --apply`。",
        )
...
def logs_prune(
    ...
    from selfmirror.config import load_config
    from selfmirror.logging_setup import _is_managed_log
        ...
        if action == "truncate":
            try:
                with path.open("w", encoding="utf-8") as f:
                    f.write(
                        f"# truncated by `selfmirror logs-prune` "
...
def _acquire_server_migration_guard() -> Any:
    ...
    from selfmirror.config import _project_root, load_config
...
def _bump_auth_epoch(cfg: Any) -> bool:
    ...
    db = Database(cfg.data_path / "selfmirror.db")
    try:
        db.initialize()
        db.bump_auth_epoch()
        return True
...
def _rebase_auth_fingerprint(cfg: Any) -> None:
    ...
    """
    from selfmirror.auth_core import password_fingerprint
...
def autostart_disable() -> None:
    """关闭登录系统后自动拉起后端。"""
    from selfmirror.config import load_config
    from selfmirror.runtime.autostart.guards import autostart_shadowed
...
def set_password(
    ...
    from selfmirror.auth_core import hash_password
    from selfmirror.config import load_config, save_config
    ...
    from selfmirror.config import API_AUTH_ENV_VARS
    ...
    if _auth_env:
        _print_status_panel(
            ...
            "如只想立即失效现有登录态，用 `selfmirror set-password --logout-all`。",
        )
        raise typer.Exit(code=1)
    ...
    from selfmirror.config import config_local_auth_keys
    ...
    if disable:
        ...
        save_config(cfg)
        _print_status_panel("success", "已关闭密码门禁", "重启后端 (selfmirror start) 后生效。")
        return
    ...
    _print_status_panel(
        "success",
        "已设置访问密码",
        "已立即失效所有现有登录态。请重启后端 (selfmirror start) 使新密码生效"
...
def _ensure_ext_key_config_writable() -> None:
    ...
    from selfmirror.config import API_AUTH_ENV_VARS, config_local_auth_keys
...
def ext_key_generate() -> None:
    ...
    from selfmirror.auth_core import generate_extension_access_key
    from selfmirror.config import load_config, save_config
    ...
    _print_status_panel(
        ...
        "设备访问密钥已生成",
        f"Key ID: {key_id}\n设备访问密钥（仅显示一次）:\n{full_key}\n\n"
        "总开关保持关闭；确认保存密钥后执行 `selfmirror ext-key enable`。",
    )
...
def ext_key_list() -> None:
    ...
    from selfmirror.auth_core import extension_access_key_ids
    from selfmirror.config import load_config
...
def ext_key_enable() -> None:
    ...
    if not extension_access_key_ids(cfg.api.auth.extension_access_keys):
        _print_status_panel(
            "error",
            "没有可用的设备密钥",
            "请先执行 `selfmirror ext-key generate`。",
        )
        raise typer.Exit(code=1)
...
def tls_proxy_status() -> None:
    ...
    extra = ""
    if not cfg.tls_proxy.enabled:
        extra = "（启用: selfmirror tls-proxy enable）"
...
def _start_tls_proxy_if_enabled(
    ...
    """
    from selfmirror.config import load_config, resolve_tls_cert_dir
    ...
    if not cfg.tls_proxy.enabled:
        if tls_port is not None:
            _print_status_panel(
                ...
                "执行 `selfmirror tls-proxy enable` 或在 config.toml"
                " 中设置 enabled=true 后重试。",
            )
        return None
    try:
        from selfmirror.tls_proxy import backend_connect_host, create_tls_proxy_server
    except ImportError as exc:
        _print_status_panel(
            "error",
            "TLS 代理启动失败",
            'cryptography 未安装。请安装 `pip install "selfmirror[tls]"` 后重试。',
        )
        raise typer.Exit(code=1) from exc
...
def _serve_api(*, host: str, port: int, tls_port: int | None) -> None:
    ...
    if _guided_init_completed_best_effort() is False:
        ...
        _print_status_panel(
            ...
            "容器内图形化初始化不可用，请在容器里运行 `selfmirror init`"
            "（宿主机执行 `docker exec -it selfmirror-backend selfmirror init`）。",
        )
    _warn_if_pause_on_disconnect_requires_presence()
...
def _ask_xhs_inclusion() -> bool:
    ...
    console.print(
        ...
        "`OPENBILICLAW_XHS_BOOTSTRAP_SCROLL_ROUNDS=0 selfmirror init` "
...
def _ask_dy_inclusion() -> bool:
    ...
    console.print(
        "[dim]   — 想跳过焦点抢占的话:Ctrl-C 退出,改用 "
        "`OPENBILICLAW_DY_BOOTSTRAP_SCROLL_ROUNDS=0 selfmirror init` "
        "拿浅层数据。[/dim]"
    )
    console.print()
    if not typer.confirm("准备好了吗,可以开始吗?", default=True):
...
def _ask_yt_inclusion() -> bool:
    ...
    console.print(
        "[dim]   — 想跳过焦点抢占的话:Ctrl-C 退出,改用 "
        "`OPENBILICLAW_YT_BOOTSTRAP_SCROLL_ROUNDS=0 selfmirror init` "
        "拿浅层数据。[/dim]"
    )
    console.print()
    if not typer.confirm("准备好了吗,可以开始吗?", default=True):
...
def _maybe_setup_password_in_init(*, allow_lan: bool) -> None:
    ...
    console.print("[dim]后续可用 `selfmirror set-password` 设置或修改。[/dim]")
    console.print()
    ...
    try:
        ...
        console.print("[yellow]密码设置失败，可稍后用 `selfmirror set-password` 重试。[/yellow]")
...
def _maybe_update_init_source_shares(event_counts: Mapping[str, int]) -> None:
    ...
    try:
        from selfmirror.config import load_config, save_config
        from selfmirror.runtime.source_policy import source_enabled_map
...
def _merge_source_shares(
    ...
    from selfmirror.runtime.source_policy import DEFAULT_POOL_SOURCE_SHARES, SOURCE_ORDER
...
async def _fetch_bilibili_init_data(
    ...
    *,
    ...
    """
    from selfmirror.bilibili.api import favorite_item_is_dead
...
async def _fetch_x_init_data(
    *,
    ...
    from selfmirror.sources.x_auth import resolve_x_cookie
    ...
    from selfmirror.sources.x_client import XClient
    from selfmirror.storage.database import Database
    ...
    try:
        health_db = Database(cfg.data_path / "selfmirror.db")
...
async def _fetch_bangumi_init_data(
    *,
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.bangumi import fetch_bangumi_public_collection_events
...
async def run_guided_init(
    *,
    ...
    try:
        from selfmirror.sources.source_bootstrap import seed_guided_init_attempts
    ...
    # selfmirror.log — so without this the desktop log shows only the eta
    ...
    try:
        ...
        message = (
            f"偏好分析失败：{llm_reason}"
            if llm_reason
            else "偏好分析阶段出错。可稍后手动重试 `selfmirror init`。"
        )
    ...
    try:
        ...
        if isinstance(exc, TimeoutError):
            ...
            message = (
                f"画像生成失败：{llm_reason}"
                if llm_reason
                else "画像生成阶段出错。可稍后手动重试 `selfmirror init`。"
            )
...
def init(
    ...
    if result.discover_exc is not None:
        _print_status_panel(
            "warning",
            "部分完成",
            result.discovery_detail + " 也可稍后手动执行 `selfmirror discover`。",
        )
    if dy_degraded:
        _print_status_panel(
            "warning",
    ...
    if weibo_status in {"failed", "timeout", "login_required"}:
        _print_status_panel(
            ...
            "再用 `selfmirror init --yes-weibo` 重试。",
        )
    ...
    if (xhs_saved + xhs_liked + xhs_history) == 0 and xhs_status != "skipped":
        console.print(
            ...
            "[cyan]selfmirror init --yes-xhs[/cyan] 可补齐。[/dim]"
        )
    if (yt_history_count + yt_subs_count + yt_likes_count) == 0 and yt_status != "skipped":
        console.print(
            ...
            "[cyan]selfmirror init --yes-youtube[/cyan] 可补齐。[/dim]"
        )
    if (
        ...
        console.print(
            ...
            "[cyan]selfmirror init --yes-zhihu[/cyan] 可补齐。[/dim]"
        )
    if (
        ...
        console.print(
            ...
            "装好扩展后重新跑 [cyan]selfmirror init --yes-reddit[/cyan] 可补齐。[/dim]"
        )
    if not bangumi_events and bangumi_status not in {"skipped", "missing_username"}:
        console.print(
            "[dim]ℹ️  Bangumi 0 条信号入库。请确认用户名存在，且收藏已设为公开。"
            "可用 [cyan]selfmirror fetch-bangumi --username <name>[/cyan] 只读验证。[/dim]"
        )
    if (
        ...
        console.print(
            ...
            "https://linux.do，然后重跑 [cyan]selfmirror init --yes-linuxdo[/cyan]。[/dim]"
        )
    if not v2ex_events and v2ex_status not in {
        ...
        console.print(
            ...
            "可用 [cyan]selfmirror init --yes-v2ex[/cyan] 重试。[/dim]"
        )
    if weibo_favorites + weibo_following + weibo_mentions == 0 and weibo_status != "skipped":
        console.print(
            ...
            "[cyan]selfmirror init --yes-weibo[/cyan]。[/dim]"
        )
    ...
    # 评估" rather than having to manually run `selfmirror cost`.
...
def _print_init_cost_summary(since_id: int) -> None:
    ...
    console.print(
        "[dim]💡 想看历史累积花费跑 `selfmirror cost` (默认 7 天) / "
        "`selfmirror cost --by caller --days 30` 看 30 天按模块拆分。"
...
def rebuild_profile(
    ...
    if not events:
        console.print(
            ...
            + "请先运行 [cyan]selfmirror init[/cyan] 拉取数据。[/yellow]"
        )
        raise typer.Exit(code=1)
...
def profile_consolidate(
    ...
    from selfmirror.llm.registry import build_embedding_service
    from selfmirror.llm.service import LLMService, module_overrides_from_config
    from selfmirror.soul.consolidator import ProfileConsolidator
    ...
    if migrate_categories:
        from selfmirror.soul.category_migration import CategoryMigrator
...
def fetch_douyin(
    ...
    前提:
      1. ``selfmirror start`` daemon 在跑(kick 才有人接)
...
def search_douyin(
    ...
    from selfmirror.discovery.douyin import split_csv_values
...
def _run_reddit_discovery_smoke(
    *,
    ...
    prechecked_status: Any | None = None
    if not _is_reddit_extension_backend(selected_backend_option):
        from selfmirror.sources.reddit_tasks import probe_reddit_command_backend
...
def fetch_x(
    ...
    console.print(
        ...
        " 跑 `selfmirror rebuild-profile` 让画像吃进新信号。"
    )
...
def import_youtube(
    ...
    """
    from selfmirror.youtube.takeout import parse_takeout
    ...
    console.print(
        ...
        "  运行 [cyan]selfmirror profile[/cyan] 查看更新后的用户画像。"
    )
...
def recommend() -> None:
    """查看推荐内容."""
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    ...
    try:
        ...
        console.print("请先执行 `selfmirror init` 拉取历史并生成初始画像。")
        raise typer.Exit(code=1) from exc
...
def keyword_inspiration_dry_run(
    ...
    from selfmirror.config import derive_inspiration_breadth_params, load_config
    from selfmirror.discovery.douyin import split_csv_values
    from selfmirror.discovery.inspiration_provider import (
        build_inspiration_search_provider,
        build_platform_source_backends,
    )
    from selfmirror.llm.service import LLMService, module_overrides_from_config
    from selfmirror.runtime.keyword_planner import KeywordPlanner
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    ...
    try:
        ...
        _print_status_panel(
            "warning",
            "尚未初始化用户画像",
            "请先执行 `selfmirror init` 拉取历史并生成初始画像。",
        )
        raise typer.Exit(code=1) from exc
    ...
    async def _preview() -> dict[str, object]:
        ...
        if bool(getattr(v2ex_cfg, "enabled", False)):
            from selfmirror.sources.v2ex_client import V2EXClient
...
def _run_xhs_discovery(*, force: bool) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.llm.service import LLMService, module_overrides_from_config
    from selfmirror.runtime.xhs_producer import XhsTaskProducer
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    from selfmirror.sources.xhs_tasks import XhsTaskQueue
...
def _recent_douyin_creator_sec_uids(*, limit: int = 20) -> tuple[str, ...]:
    ...
    if not hasattr(database, "conn"):
        return ()
    try:
        from selfmirror.sources.dy_tasks import recent_dy_creator_sec_uids
...
def _run_douyin_discovery(
    *,
    ...
    import selfmirror.config as config_module
    from selfmirror.discovery.douyin import (
    ...
    from selfmirror.sources.douyin_direct import DouyinDirectAuthError, DouyinDirectClient
    from selfmirror.sources.douyin_plugin_search import DouyinPluginSearchClient
...
def _build_discovery_candidate_pipeline(
    *,
    ...
    from selfmirror.discovery.candidate_pipeline import DiscoveryCandidatePipeline
...
def _run_douyin_formal_discovery(*, limit: int) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.douyin_producer import build_douyin_discovery_producer
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    from selfmirror.sources.douyin_auth import resolve_douyin_cookie
    ...
    messages = {
        ...
        "no_profile": ("warning", "尚未初始化 Soul 画像", "请先执行 `selfmirror init`。"),
        "budget_exhausted": (
            "warning",
            "抖音 discovery 分支预算已耗尽",
...
def _run_zhihu_discovery(*, limit: int) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.runtime.zhihu_producer import build_zhihu_discovery_producer
    from selfmirror.soul.engine import SoulProfileNotInitializedError
...
def _run_linuxdo_discovery(*, limit: int) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.runtime.linuxdo_producer import build_linuxdo_discovery_producer
    from selfmirror.soul.engine import SoulProfileNotInitializedError
...
def _run_reddit_discovery(*, limit: int) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.runtime.reddit_producer import build_reddit_discovery_producer
    from selfmirror.soul.engine import SoulProfileNotInitializedError
...
def _run_bangumi_discovery_smoke(*, mode: str, keyword: str = "", limit: int) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.bangumi import bangumi_subject_to_content
    from selfmirror.sources.bangumi_client import BangumiAPIError, BangumiClient
...
def _run_v2ex_discovery_smoke(
    *, mode: str, keyword: str = "", node: str = "", tab: str = "", limit: int
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.v2ex import v2ex_topic_to_content
    from selfmirror.sources.v2ex_client import V2EXAPIError, V2EXClient
...
def _run_v2ex_discovery(*, limit: int, force: bool = False) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.runtime.v2ex_producer import (
    ...
    try:
        ...
        _print_status_panel("warning", "尚未初始化用户画像", "请先执行 `selfmirror init`。")
        raise typer.Exit(code=1) from exc
    discovery_engine = _build_discovery_engine()
    candidate_pipeline = _build_discovery_candidate_pipeline(
    ...
    messages = {
        ...
        "no_profile": ("warning", "尚未初始化 V2EX 画像", "请先执行 `selfmirror init`。"),
...
def _run_bangumi_discovery(*, limit: int, force: bool = False) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.bangumi_producer import BangumiDiscoveryProducer
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    from selfmirror.sources.bangumi_client import BangumiClient
...
def _run_weibo_discovery_smoke(
    *,
    ...
    from selfmirror.config import load_config
    from selfmirror.sources.weibo import (
        weibo_hot_topic_query,
        weibo_post_to_content,
    )
    from selfmirror.sources.weibo_client import WeiboClient, WeiboClientError
...
def _run_weibo_discovery(*, limit: int, force: bool = False) -> None:
    ...
    from selfmirror.config import load_config
    from selfmirror.runtime.keyword_fetch import KeywordFetchCoordinator
    from selfmirror.runtime.weibo_producer import WeiboDiscoveryProducer
    from selfmirror.soul.engine import SoulProfileNotInitializedError
    from selfmirror.sources.weibo_client import WeiboClient
...
def delight() -> None:
    """手动触发一次惊喜推荐检查."""
    from selfmirror.recommendation.delight import effective_delight_threshold
    from selfmirror.soul.engine import SoulProfileNotInitializedError
...
def config_show() -> None:
    """显示当前配置."""
    from selfmirror.config import effective_llm_default_chain, load_config_with_diagnostics
    from selfmirror.llm import RegistryBuildError, summarize_registry
...
def login_codex(
    ...
    from selfmirror.llm.codex_auth import (
        CodexAuthError,
        CodexCredentials,
    ...
    if status:
        ...
        if credentials is None:
            _print_status_panel(
                "warning",
                "Codex OAuth",
                "未登录。请运行 `selfmirror login codex` "
                "或 `selfmirror login codex --import`。",
            )
            return
...
def health_check() -> None:
    ...
    from selfmirror.llm import RegistryBuildError
...
def browser_open(url: str) -> None:
    ...
    from selfmirror.bilibili.browser import BrowserCommandError
